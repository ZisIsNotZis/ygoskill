...
YGOROOTDIY = getenv('YGOROOTDIY', 'noexist') #./expansions
        ...
        for i in open(i).read().split('\n!')[:0:-1]:
            ...
            for i, lfs in LF[ot].items():
                if i not in lf and lfs and list(lfs.values())[-1] != 3:
                    LF[ot][i][t] = 3
SET = {int(i, 16): j.split('|')for i in (f'{YGOROOTDIY}/strings.conf', f'{YGOROOT}/strings.conf')if path.exists(i)for i in open(i).read().split('setname ')[1:]for i, j in [i.split()[:2]]}
...
for i in argv[1:]:
    if not(i.endswith('.ydk')or i.endswith('/')):
        i = compile(i)
        print(*(j for j in CARD.values()if i.findall(j)), sep='\n', end='\n\n')