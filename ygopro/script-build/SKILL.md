...
Implement a YGOPro Lua script for a card effect from scratch or from a specification. Follow every rule below -- no improvisation, no shortcuts, no making up functions or constants.
...
- Always use one of these two styles -- do not use bare `function initial_effect(c)` or unscoped `function MyCondition(e,tp,eg,ep,ev,re,r,rp)`
...
- Do not make up a function that does not exist in the API docs -- if unsure, check [api/](../api/SKILL.md) or search script/utility.lua and script/procedure.lua
- Do not make up a constant that does not exist in script/constant.lua -- check [api/constants.md](../api/constants.md) or [api/consteffect.md](../api/consteffect.md)
- Do not use mock, stub, placeholder, or ellipsis in implementation -- every line must be real, correct, and complete
- Do not skip an effect because it seems complicated -- implement it fully using the real API
- Do not imagine a helper function exists -- all helpers live in aux (script/utility.lua) or script/procedure.lua, verify before using
...
- **Output Format**
  ...
  Script: c{id}.lua
  Effects:
    - Effect 1: type={EFFECT_TYPE_*}, event={EVENT_*}, range={range}, property={property}
    - Effect 2: ...
  Callbacks: {condition/cost/target/operation functions}
